...
static float sentrySpeed = 200.0;

static int enemySize = 62;
...
static void UpdateSaw(Entity* self)
{
	...
	int mx, my;
	mx = self->direction == LEFT ? ((self->pos.x - 1) *INV_TILE_SIZE) : (self->pos.x + self->w + 10) *INV_TILE_SIZE;
	my = self->pos.y * INV_TILE_SIZE;
...
static void GoBackAndForth(Entity* self)
{
	...
	mx = self->pos.x * INV_TILE_SIZE;
	my = (self->pos.y + self->h+1) * INV_TILE_SIZE;
	if (IsLedgeTile(mx,my,1,self->direction))
	{
		self->direction = self->direction == LEFT ? RIGHT : LEFT;
	}
}
...
static void UpdateShooter(Entity* self)
{
	...
	if (bulVel.x != 0 && SDL_GetTicks() - self->lastTime > 2000)
	{
