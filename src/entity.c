...
{
	SDL_Rect screenRect = {
		.x = e->pos.x-game.camera.x,
		.y=e->pos.y-game.camera.y,
		.w=e->w,
		.h=e->h
	};
	...
	SDL_RenderFillRect(game.renderer, &screenRect);
...
static void ResolveSolidCollisions(Entity* self, Entity* other)
{
	...
	if (other->isSolid)
	{
		...
		adjustmentx = self->vel.x > 0 ? -((self->pos.x + self->w) - (other->pos.x)) : (other->pos.x + other->w) - (self->pos.x);
		adjustmenty = self->vel.y > 0 ? -((self->pos.y + self->h) - (other->pos.y)) : (other->pos.y + other->h) - (self->pos.y);
